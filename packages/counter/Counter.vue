...
export default {
  ...
  watch: {
    count: {
      immediate: true,
      handler(num) {
        if (num === this.min) this.minusColor = this.disabledColor
        else if (num < this.min) this.$emit('change', this.min)
        else this.minusColor = this.fillColor
        if (num === this.max) this.plusColor = this.disabledColor
        else if (num > this.max) this.$emit('change', this.max)
